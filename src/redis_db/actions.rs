...
use redis::{pipe, Client, Connection, transaction, Commands};
    ...
    fn store(&mut self, fingerprints: &Vec<usize>, song: &String) -> Result<(), Box<dyn Error>> {
        transaction(&mut self.connection, fingerprints, |con, pipe| {
            for fingerprint in fingerprints.iter() {
                pipe
                    .sadd(*fingerprint, song.clone()).ignore();
            }
...
mod test {
    ...
    fn test_benchmark_repository() {
        ...
        println!("Adding to database of 2 songs of total {} fingerprints took {} milliseconds", fingerprint_1_max + fingerprint_2_max, start_time.elapsed().as_millis());
        ...
        println!("Matching of total {} fingerprints took {} milliseconds", fingerprint_1_max + fingerprint_2_max, start_time.elapsed().as_millis());
    }
}
